...
export const addConsumedProduct = async (req, res, next) => {
  try {
    ...
    const product = await Products.findOne({
      title: { $regex: new RegExp(`^${formattedConsumedProduct}$`, 'i') },
    });
...
export const getSummaryData = async (req, res, next) => {
  try {
    const userId = req.user.id; // Assuming you're using authentication and req.user is populated
    const { date } = req.params; // Date passed as a parameter to fetch summary for a specific date

    // Fetch user data from MongoDB
    const user = await Users.findById(userId).select(
      'dailyConsumedProducts usersInfo'
    );
    ...
    // Calculate the total calories consumed on the given date
    const totalConsumed = user.dailyConsumedProducts
      .filter(
        product =>
    ...
    const recommendedCalories = user.usersInfo.recommendedCalories;
    const left = recommendedCalories - totalConsumed;

    // Prepare summary data to send back
    const summaryData = {
      date,
      left: `${left} kcal`,
      consumed: `${totalConsumed} kcal`,
      dailyRate: `${recommendedCalories} kcal`,
      normalPercentage:
        ((totalConsumed / recommendedCalories) * 100).toFixed(2) + '%',
      foodNotRecommended:
        user.usersInfo.foodsNotRecommended.join(', ') ||
        'No foods not recommended',
    };
